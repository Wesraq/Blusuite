...
def company_list(request):
    ...
    if not (request.user.role == 'ADMIN' or request.user.role == 'SUPERADMIN'):
        messages.error(request, 'Access denied. Superuser Admin privileges required.')
        return redirect('/')
    ...
    # Debug: Print counts
    print(f"DEBUG - Pending requests: {pending_requests.count()}")
    print(f"DEBUG - Approved companies: {approved_companies.count()}")
    print(f"DEBUG - Rejected companies: {rejected_companies.count()}")
...
def company_edit(request, company_id):
    ...
    if request.user.role not in ['ADMIN', 'SUPERADMIN', 'ADMINISTRATOR', 'EMPLOYER_ADMIN']:
    ...
    if request.method == 'POST':
        ...
        if 'primary_color' in request.POST and 'name' not in request.POST:
            try:
