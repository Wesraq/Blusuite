...
class CustomReportBuilder:
    ...
    def __init__(self, source, company=None):
        """
        Initialize report builder
        
        Args:
            source: Report source key (e.g., 'employees', 'attendance')
            company: Company instance for filtering
        """
        self.source = source
    ...
    def build_queryset(self, filters=None):
        ...
        if self.company and hasattr(model, 'employee'):
            queryset = queryset.filter(employee__company=self.company)
        elif self.company and hasattr(model, 'company'):
...
def generate_custom_report(source, company, filters=None, selected_fields=None):
    """
