...
def integration_test(request, integration_id):
    ...
    try:
        ...
        integration = company_integration.integration
        log_request_data = {}
        log_response_data = {}
        success = False
        message = ''
        ...
        if integration.integration_type == 'SLACK':
            if not company_integration.access_token:
                raise ValueError('Slack integration is not connected yet. Please reconnect and try again.')

            slack = SlackIntegration(company_integration.access_token)

            auth_response = slack.test_connection()
            log_response_data['auth_test'] = auth_response
            success = auth_response.get('ok', False)

            if success:
                config = company_integration.config_json or {}
                default_channel = config.get('default_channel')

                if default_channel:
                    test_text = (
                        f"✅ EMS integration test successful at "
                    ...
                    message_response = slack.post_message(default_channel, text=test_text)
                    log_response_data['message_post'] = message_response

                    if message_response.get('ok'):
                        message = f'Test message sent to Slack channel {default_channel}.'
                    else:
                        success = False
                        error_detail = message_response.get('error', 'Unknown error')
                        message = f'Slack test message failed: {error_detail}.'
                else:
                    message = 'Slack connection verified. Configure a default channel to send test messages.'
            else:
                error_detail = auth_response.get('error', 'Unknown error')
                message = f'Slack auth test failed: {error_detail}.'
        else:
            success = True
        ...
        IntegrationLog.objects.create(
            company_integration=company_integration,
            action='SYNC',
            description=message or 'Connection test performed',
            success=success,
        ...
        if success:
            ...
            company_integration.save(update_fields=['last_synced_at', 'updated_at'])
        else:
            company_integration.mark_error(message or 'Integration test failed')

        return JsonResponse({
            'success': success,
            'message': message or 'Connection test completed.'
        })
