...
def form_templates_list(request):
    ...
    if request.user.role == 'EMPLOYEE' and request.user.employee_profile.employee_role not in ['HR', 'SUPERVISOR']:
    ...
    if status and request.user.employee_profile.employee_role in ['HR', 'SUPERVISOR']:
...
def form_template_delete(request, template_id):
    ...
    if not (request.user.is_employer or request.user.employee_profile.employee_role == 'HR' or template.created_by == request.user):
        messages.error(request, 'Access denied.')
        return redirect('form_templates_list')
    ...
    return render(request, 'eforms/template_confirm_delete.html', {'template': template})
...
def submission_export_pdf(request, submission_id):
    """Stub export view to satisfy URL; extend with real PDF export if needed"""
    submission = get_object_or_404(
        FormSubmission,
        id=submission_id,
        company=request.user.company
    ...
    return HttpResponse("PDF export not implemented", content_type='text/plain')
