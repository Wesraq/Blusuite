...
class DocumentCategoryListCreateView(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        return DocumentCategory.objects.filter(tenant=tenant) if tenant else DocumentCategory.objects.none()
...
class DocumentApprovalView(APIView):
    ...
    def post(self, request, pk):
        ...
        try:
            ...
            document = EmployeeDocument.objects.get(pk=pk, tenant=tenant)
        ...
        serializer = DocumentApprovalSerializer(data=request.data, context={'request': request})
...
class DocumentTemplateListCreateView(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        tenant = getattr(self.request, 'tenant', None)
        return DocumentTemplate.objects.filter(tenant=tenant, is_active=True) if tenant else DocumentTemplate.objects.none()
...
class DocumentTemplateDetailView(generics.RetrieveUpdateDestroyAPIView):
    ...
    def get_queryset(self):
        tenant = getattr(self.request, 'tenant', None)
        return DocumentTemplate.objects.filter(tenant=tenant) if tenant else DocumentTemplate.objects.none()
...
class DocumentAccessLogListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        tenant = getattr(self.request, 'tenant', None)
        queryset = DocumentAccessLog.objects.filter(tenant=tenant) if tenant else DocumentAccessLog.objects.none()
...
class DocumentStatsView(APIView):
    ...
    def get(self, request):
        ...
        # Base queryset
        if not tenant:
            return Response(DocumentStatsSerializer({}).data)
        if user.role == User.Role.EMPLOYER_ADMIN:
            documents = EmployeeDocument.objects.filter(tenant=tenant, employee__company=user.company)
        else:  # Administrator or SuperAdmin
            documents = EmployeeDocument.objects.filter(tenant=tenant)
