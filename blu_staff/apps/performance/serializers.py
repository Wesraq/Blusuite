...
class PerformanceReviewSerializer(serializers.ModelSerializer):
    ...
    def validate_employee(self, value):
        ...
        if tenant and getattr(value.company, 'tenant_id', None) != tenant.id:
            raise serializers.ValidationError('Selected employee does not belong to this tenant.')
        return value
...
class PerformanceGoalSerializer(serializers.ModelSerializer):
    ...
    def validate_review(self, value):
        ...
        if tenant and value.tenant_id != tenant.id:
            raise serializers.ValidationError('Review does not belong to this tenant.')
        return value
...
class PerformanceTemplateSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        tenant = getattr(self.context['request'], 'tenant', None)
        if tenant and tenant.performance_template_items.filter(name=data.get('name')).exists():
            raise serializers.ValidationError({'name': 'Template name already exists for this tenant.'})
        return data
