from django.db import models
from django.contrib.auth import get_user_model
from django.utils.translation import gettext_lazy as _
from django.core.exceptions import ValidationError
from django.utils import timezone

from tenant_management.models import TenantScopedModel

User = get_user_model()


class PerformanceReview(TenantScopedModel):
    """Model for employee performance reviews"""

    class ReviewType(models.TextChoices):
        QUARTERLY = 'QUARTERLY', _('Quarterly Review')
        MID_YEAR = 'MID_YEAR', _('Mid-Year Review')
        ANNUAL = 'ANNUAL', _('Annual Review')
        PROBATION = 'PROBATION', _('Probation Review')
        SPECIAL = 'SPECIAL', _('Special Review')

    class OverallRating(models.TextChoices):
        OUTSTANDING = 'OUTSTANDING', _('Outstanding')
        EXCEEDS_EXPECTATIONS = 'EXCEEDS_EXPECTATIONS', _('Exceeds Expectations')
        MEETS_EXPECTATIONS = 'MEETS_EXPECTATIONS', _('Meets Expectations')
        BELOW_EXPECTATIONS = 'BELOW_EXPECTATIONS', _('Below Expectations')
        UNSATISFACTORY = 'UNSATISFACTORY', _('Unsatisfactory')

    cycle = models.ForeignKey(
        'PerformanceReviewCycle',
        on_delete=models.SET_NULL,
        null=True,
        blank=True,
        related_name='reviews',
        help_text=_('Review cycle this review belongs to')
    )
    employee = models.ForeignKey(
        User,
        on_delete=models.CASCADE,
        related_name='perf_reviews',
        limit_choices_to={'role': 'EMPLOYEE'}
    )
    reviewer = models.ForeignKey(
        User,
        on_delete=models.SET_NULL,
        null=True,
        related_name='perf_conducted_reviews',
        limit_choices_to={'role__in': ['ADMIN', 'EMPLOYER_SUPERUSER', 'EMPLOYER_ADMIN']}
    )
    review_type = models.CharField(
        _('review type'),
        max_length=20,
        choices=ReviewType.choices,
        default=ReviewType.QUARTERLY
    )
    review_period_start = models.DateField(_('review period start'))
    review_period_end = models.DateField(_('review period end'))
    review_date = models.DateField(_('review date'), default=timezone.now)
    overall_rating = models.CharField(
        _('overall rating'),
        max_length=25,
        choices=OverallRating.choices,
        default=OverallRating.MEETS_EXPECTATIONS
    )
    title = models.CharField(_('review title'), max_length=200)
    objectives = models.TextField(_('review objectives'), blank=True)
    
    # Employee Self-Assessment Fields
    employee_comments = models.TextField(_('employee comments'), blank=True)
    self_rating = models.CharField(
        _('self rating'),
        max_length=25,
        choices=OverallRating.choices,
        blank=True,
        null=True
    )
    achievements = models.TextField(_('achievements'), blank=True)
    challenges_faced = models.TextField(_('challenges faced'), blank=True)
    training_needs = models.TextField(_('training needs'), blank=True)
    goals_next_period = models.TextField(_('goals for next period'), blank=True)
    
    # Reviewer Feedback Fields
    reviewer_comments = models.TextField(_('reviewer comments'), blank=True)
    strengths = models.TextField(_('strengths'), blank=True)
    areas_for_improvement = models.TextField(_('areas for improvement'), blank=True)
    development_plan = models.TextField(_('development plan'), blank=True)
    recommended_actions = models.TextField(_('recommended actions'), blank=True)
    
    # Legacy field
    additional_comments = models.TextField(_('additional comments'), blank=True)
    
    # Workflow Timestamps
    employee_submitted_at = models.DateTimeField(_('employee submitted at'), null=True, blank=True)
    reviewer_completed_at = models.DateTimeField(_('reviewer completed at'), null=True, blank=True)
    
    status = models.CharField(
        _('status'),
        max_length=20,
        choices=[
            ('DRAFT', _('Draft')),
            ('SUBMITTED', _('Submitted')),
            ('UNDER_REVIEW', _('Under Review')),
            ('COMPLETED', _('Completed')),
            ('APPROVED', _('Approved'))
        ],
        default='DRAFT'
    )
    is_confidential = models.BooleanField(_('confidential'), default=True)
    created_at = models.DateTimeField(_('created at'), auto_now_add=True)
    updated_at = models.DateTimeField(_('updated at'), auto_now=True)

    class Meta:
        app_label = 'performance'
        verbose_name = _('performance review')
        verbose_name_plural = _('performance reviews')
        ordering = ['-review_date', '-created_at']

    def __str__(self):
        return f"{self.employee.get_full_name()} - {self.get_review_type_display()} - {self.review_date}"


class PerformanceGoal(TenantScopedModel):
    """Model for individual performance goals"""

    class Status(models.TextChoices):
        NOT_STARTED = 'NOT_STARTED', _('Not Started')
        IN_PROGRESS = 'IN_PROGRESS', _('In Progress')
        COMPLETED = 'COMPLETED', _('Completed')
        CANCELLED = 'CANCELLED', _('Cancelled')

    class Priority(models.TextChoices):
        LOW = 'LOW', _('Low')
        MEDIUM = 'MEDIUM', _('Medium')
        HIGH = 'HIGH', _('High')
        CRITICAL = 'CRITICAL', _('Critical')

    review = models.ForeignKey(
        PerformanceReview,
        on_delete=models.CASCADE,
        related_name='goals'
    )
    title = models.CharField(_('goal title'), max_length=200)
    description = models.TextField(_('goal description'), blank=True)
    category = models.CharField(_('category'), max_length=100, blank=True)
    priority = models.CharField(
        _('priority'),
        max_length=20,
        choices=Priority.choices,
        default=Priority.MEDIUM
    )
    status = models.CharField(
        _('status'),
        max_length=20,
        choices=Status.choices,
        default=Status.NOT_STARTED
    )
    progress_percentage = models.PositiveIntegerField(
        _('progress percentage'),
        default=0,
        help_text=_('Progress from 0 to 100%')
    )
    target_completion_date = models.DateField(_('target completion date'), null=True, blank=True)
    actual_completion_date = models.DateField(_('actual completion date'), null=True, blank=True)
    success_criteria = models.TextField(_('success criteria'), blank=True)
    challenges = models.TextField(_('challenges faced'), blank=True)
    support_needed = models.TextField(_('support needed'), blank=True)
    created_at = models.DateTimeField(_('created at'), auto_now_add=True)
    updated_at = models.DateTimeField(_('updated at'), auto_now=True)

    class Meta:
        app_label = 'performance'
        verbose_name = _('performance goal')
        verbose_name_plural = _('performance goals')
        ordering = ['priority', 'created_at']

    def __str__(self):
        return f"{self.review.employee.get_full_name()} - {self.title}"


class PerformanceMetric(TenantScopedModel):
    """Model for quantitative performance metrics"""

    class MetricType(models.TextChoices):
        NUMERIC = 'NUMERIC', _('Numeric')
        PERCENTAGE = 'PERCENTAGE', _('Percentage')
        CURRENCY = 'CURRENCY', _('Currency')
        TIME = 'TIME', _('Time-based')

    review = models.ForeignKey(
        PerformanceReview,
        on_delete=models.CASCADE,
        related_name='metrics'
    )
    name = models.CharField(_('metric name'), max_length=100)
    description = models.TextField(_('description'), blank=True)
    metric_type = models.CharField(
        _('metric type'),
        max_length=20,
        choices=MetricType.choices,
        default=MetricType.NUMERIC
    )
    target_value = models.DecimalField(_('target value'), max_digits=15, decimal_places=2)
    actual_value = models.DecimalField(
        _('actual value'),
        max_digits=15,
        decimal_places=2,
        null=True,
        blank=True
    )
    unit = models.CharField(_('unit'), max_length=50, blank=True)
    weight = models.PositiveIntegerField(
        _('weight'),
        default=100,
        help_text=_('Relative weight of this metric in the overall review')
    )
    is_achieved = models.BooleanField(_('achieved'), default=False)
    created_at = models.DateTimeField(_('created at'), auto_now_add=True)
    updated_at = models.DateTimeField(_('updated at'), auto_now=True)

    class Meta:
        app_label = 'performance'
        verbose_name = _('performance metric')
        verbose_name_plural = _('performance metrics')
        ordering = ['weight', 'name']

    def __str__(self):
        return f"{self.review.employee.get_full_name()} - {self.name}"
    
    @property
    def achievement_percentage(self):
        """Calculate achievement percentage"""
        if self.target_value and self.actual_value is not None:
            if self.target_value != 0:
                return min((self.actual_value / self.target_value) * 100, 1000)
        return 0

    @property
    def is_target_achieved(self):
        """Check if target is achieved"""
        if self.actual_value is not None and self.target_value:
            return self.actual_value >= self.target_value
        return False


class PerformanceFeedback(TenantScopedModel):
    """Model for feedback from colleagues and managers"""

    class FeedbackType(models.TextChoices):
        SELF_ASSESSMENT = 'SELF', _('Self Assessment')
        MANAGER_FEEDBACK = 'MANAGER', _('Manager Feedback')
        PEER_FEEDBACK = 'PEER', _('Peer Feedback')
        SUBORDINATE_FEEDBACK = 'SUBORDINATE', _('Subordinate Feedback')
        CUSTOMER_FEEDBACK = 'CUSTOMER', _('Customer Feedback')

    review = models.ForeignKey(
        PerformanceReview,
        on_delete=models.CASCADE,
        related_name='feedback'
    )
    feedback_type = models.CharField(
        _('feedback type'),
        max_length=20,
        choices=FeedbackType.choices,
        default=FeedbackType.MANAGER_FEEDBACK
    )
    provided_by = models.ForeignKey(
        User,
        on_delete=models.SET_NULL,
        null=True,
        related_name='perf_provided_feedback'
    )
    relationship_to_employee = models.CharField(
        _('relationship to employee'),
        max_length=100,
        blank=True,
        help_text=_('e.g., Direct Manager, Colleague, Team Member')
    )
    rating = models.PositiveIntegerField(
        _('rating'),
        help_text=_('Rating from 1 to 5')
    )
    strengths = models.TextField(_('strengths'), blank=True)
    areas_for_improvement = models.TextField(_('areas for improvement'), blank=True)
    overall_comments = models.TextField(_('overall comments'), blank=True)
    suggestions = models.TextField(_('suggestions'), blank=True)
    is_anonymous = models.BooleanField(_('anonymous'), default=False)
    created_at = models.DateTimeField(_('created at'), auto_now_add=True)
    updated_at = models.DateTimeField(_('updated at'), auto_now=True)

    class Meta:
        app_label = 'performance'
        verbose_name = _('performance feedback')
        verbose_name_plural = _('performance feedback')
        ordering = ['-created_at']

    def __str__(self):
        return f"{self.get_feedback_type_display()} - {self.review.employee.get_full_name()}"


class PerformanceTemplate(TenantScopedModel):
    """Model for performance review templates"""

    name = models.CharField(_('template name'), max_length=200)
    description = models.TextField(_('description'), blank=True)
    review_type = models.CharField(
        _('review type'),
        max_length=20,
        choices=PerformanceReview.ReviewType.choices,
        default=PerformanceReview.ReviewType.QUARTERLY
    )
    is_default = models.BooleanField(_('default template'), default=False)
    sections = models.JSONField(
        _('template sections'),
        default=dict,
        help_text=_('JSON structure defining template sections and fields')
    )
    created_by = models.ForeignKey(
        User,
        on_delete=models.SET_NULL,
        null=True,
        related_name='perf_created_templates'
    )
    is_active = models.BooleanField(_('active'), default=True)
    created_at = models.DateTimeField(_('created at'), auto_now_add=True)
    updated_at = models.DateTimeField(_('updated at'), auto_now=True)

    class Meta:
        app_label = 'performance'
        verbose_name = _('performance template')
        verbose_name_plural = _('performance templates')
        ordering = ['-is_default', 'name']

    def __str__(self):
        return f"{self.name} ({self.get_review_type_display()})"


class PerformanceReviewCycle(TenantScopedModel):
    """Model for managing performance review cycles/periods"""
    
    class CycleStatus(models.TextChoices):
        DRAFT = 'DRAFT', _('Draft')
        ACTIVE = 'ACTIVE', _('Active')
        IN_PROGRESS = 'IN_PROGRESS', _('In Progress')
        COMPLETED = 'COMPLETED', _('Completed')
        CANCELLED = 'CANCELLED', _('Cancelled')
    
    name = models.CharField(_('cycle name'), max_length=200, help_text=_('e.g., Q1 2024 Reviews, Annual 2024'))
    description = models.TextField(_('description'), blank=True)
    review_type = models.CharField(
        _('review type'),
        max_length=20,
        choices=PerformanceReview.ReviewType.choices,
        default=PerformanceReview.ReviewType.QUARTERLY
    )
    start_date = models.DateField(_('cycle start date'))
    end_date = models.DateField(_('cycle end date'))
    review_period_start = models.DateField(_('review period start'), help_text=_('Period being reviewed'))
    review_period_end = models.DateField(_('review period end'), help_text=_('Period being reviewed'))
    deadline = models.DateField(_('review deadline'), help_text=_('Deadline for completing reviews'))
    
    template = models.ForeignKey(
        PerformanceTemplate,
        on_delete=models.SET_NULL,
        null=True,
        blank=True,
        related_name='cycles',
        help_text=_('Template to use for reviews in this cycle')
    )
    
    status = models.CharField(
        _('status'),
        max_length=20,
        choices=CycleStatus.choices,
        default=CycleStatus.DRAFT
    )
    
    auto_assign_reviewers = models.BooleanField(
        _('auto-assign reviewers'),
        default=True,
        help_text=_('Automatically assign direct managers as reviewers')
    )
    
    send_reminders = models.BooleanField(
        _('send reminders'),
        default=True,
        help_text=_('Send reminder emails to reviewers')
    )
    
    reminder_days_before = models.PositiveIntegerField(
        _('reminder days before deadline'),
        default=7,
        help_text=_('Send reminders this many days before deadline')
    )
    
    created_by = models.ForeignKey(
        User,
        on_delete=models.SET_NULL,
        null=True,
        related_name='perf_created_review_cycles'
    )
    
    created_at = models.DateTimeField(_('created at'), auto_now_add=True)
    updated_at = models.DateTimeField(_('updated at'), auto_now=True)
    
    class Meta:
        app_label = 'performance'
        verbose_name = _('performance review cycle')
        verbose_name_plural = _('performance review cycles')
        ordering = ['-start_date', '-created_at']
    
    def __str__(self):
        return f"{self.name} ({self.get_review_type_display()})"
    
    @property
    def total_assignments(self):
        return self.assignments.count()
    
    @property
    def completed_reviews(self):
        return self.assignments.filter(review__status__in=['COMPLETED', 'APPROVED']).count()
    
    @property
    def completion_percentage(self):
        total = self.total_assignments
        if total == 0:
            return 0
        return round((self.completed_reviews / total) * 100, 1)
    
    @property
    def is_overdue(self):
        from datetime import date
        return self.status == 'IN_PROGRESS' and date.today() > self.deadline


class ReviewCycleAssignment(TenantScopedModel):
    """Model for assigning employees to review cycles"""
    
    class AssignmentStatus(models.TextChoices):
        PENDING = 'PENDING', _('Pending')
        IN_PROGRESS = 'IN_PROGRESS', _('In Progress')
        COMPLETED = 'COMPLETED', _('Completed')
        SKIPPED = 'SKIPPED', _('Skipped')
    
    cycle = models.ForeignKey(
        PerformanceReviewCycle,
        on_delete=models.CASCADE,
        related_name='assignments'
    )
    
    employee = models.ForeignKey(
        User,
        on_delete=models.CASCADE,
        related_name='perf_review_assignments',
        limit_choices_to={'role': 'EMPLOYEE'}
    )
    
    reviewer = models.ForeignKey(
        User,
        on_delete=models.SET_NULL,
        null=True,
        blank=True,
        related_name='perf_assigned_reviews',
        help_text=_('Assigned reviewer for this employee')
    )
    
    review = models.OneToOneField(
        PerformanceReview,
        on_delete=models.SET_NULL,
        null=True,
        blank=True,
        related_name='cycle_assignment',
        help_text=_('Created review for this assignment')
    )
    
    status = models.CharField(
        _('status'),
        max_length=20,
        choices=AssignmentStatus.choices,
        default=AssignmentStatus.PENDING
    )
    
    notes = models.TextField(_('notes'), blank=True)
    
    reminder_sent = models.BooleanField(_('reminder sent'), default=False)
    reminder_sent_at = models.DateTimeField(_('reminder sent at'), null=True, blank=True)
    
    created_at = models.DateTimeField(_('created at'), auto_now_add=True)
    updated_at = models.DateTimeField(_('updated at'), auto_now=True)
    
    class Meta:
        app_label = 'performance'
        verbose_name = _('review cycle assignment')
        verbose_name_plural = _('review cycle assignments')
        ordering = ['cycle', 'employee__last_name', 'employee__first_name']
    
    def __str__(self):
        return f"{self.cycle.name} - {self.employee.get_full_name()}"


class CompetencyFramework(TenantScopedModel):
    """Model for defining competency frameworks"""
    
    name = models.CharField(_('framework name'), max_length=200)
    description = models.TextField(_('description'), blank=True)
    is_active = models.BooleanField(_('active'), default=True)
    is_default = models.BooleanField(_('default framework'), default=False)
    
    created_by = models.ForeignKey(
        User,
        on_delete=models.SET_NULL,
        null=True,
        related_name='perf_created_competency_frameworks'
    )
    
    created_at = models.DateTimeField(_('created at'), auto_now_add=True)
    updated_at = models.DateTimeField(_('updated at'), auto_now=True)
    
    class Meta:
        app_label = 'performance'
        verbose_name = _('competency framework')
        verbose_name_plural = _('competency frameworks')
        ordering = ['-is_default', 'name']
    
    def __str__(self):
        return self.name


class Competency(TenantScopedModel):
    """Model for individual competencies"""
    
    class CompetencyType(models.TextChoices):
        TECHNICAL = 'TECHNICAL', _('Technical Skill')
        BEHAVIORAL = 'BEHAVIORAL', _('Behavioral')
        LEADERSHIP = 'LEADERSHIP', _('Leadership')
        COMMUNICATION = 'COMMUNICATION', _('Communication')
        CORE_VALUE = 'CORE_VALUE', _('Core Value')
    
    framework = models.ForeignKey(
        CompetencyFramework,
        on_delete=models.CASCADE,
        related_name='competencies'
    )
    
    name = models.CharField(_('competency name'), max_length=200)
    description = models.TextField(_('description'))
    competency_type = models.CharField(
        _('type'),
        max_length=20,
        choices=CompetencyType.choices,
        default=CompetencyType.BEHAVIORAL
    )
    
    level_1_description = models.TextField(_('level 1 - developing'), blank=True)
    level_2_description = models.TextField(_('level 2 - competent'), blank=True)
    level_3_description = models.TextField(_('level 3 - proficient'), blank=True)
    level_4_description = models.TextField(_('level 4 - expert'), blank=True)
    level_5_description = models.TextField(_('level 5 - master'), blank=True)
    
    weight = models.PositiveIntegerField(_('weight'), default=100)
    is_required = models.BooleanField(_('required'), default=True)
    
    created_at = models.DateTimeField(_('created at'), auto_now_add=True)
    updated_at = models.DateTimeField(_('updated at'), auto_now=True)
    
    class Meta:
        app_label = 'performance'
        verbose_name = _('competency')
        verbose_name_plural = _('competencies')
        ordering = ['framework', 'competency_type', 'name']
    
    def __str__(self):
        return f"{self.framework.name} - {self.name}"


class CompetencyRating(TenantScopedModel):
    """Model for rating competencies in a review"""
    
    review = models.ForeignKey(
        PerformanceReview,
        on_delete=models.CASCADE,
        related_name='competency_ratings'
    )
    
    competency = models.ForeignKey(
        Competency,
        on_delete=models.CASCADE,
        related_name='ratings'
    )
    
    rating = models.PositiveIntegerField(
        _('rating'),
        help_text=_('Rating from 1 to 5')
    )
    
    comments = models.TextField(_('comments'), blank=True)
    evidence = models.TextField(_('evidence/examples'), blank=True)
    
    created_at = models.DateTimeField(_('created at'), auto_now_add=True)
    updated_at = models.DateTimeField(_('updated at'), auto_now=True)
    
    class Meta:
        app_label = 'performance'
        verbose_name = _('competency rating')
        verbose_name_plural = _('competency ratings')
        ordering = ['review', 'competency']
    
    def __str__(self):
        return f"{self.review} - {self.competency.name}: {self.rating}/5"
