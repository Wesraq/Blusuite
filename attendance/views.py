...
from django.db.models import Q, Count, Sum, F, ExpressionWrapper, DurationField, DateField
...
class CheckInView(APIView):
    ...
    def post(self, request):
        ...
        if existing_attendance:
            serializer = AttendanceSerializer(existing_attendance, data=attendance_data, partial=True)
        else:
...
class AttendanceListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        # Employers can see their employees' attendance
...
class LeaveRequestListView(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        if user.role == User.Role.EMPLOYEE:
            queryset = queryset.filter(employee=user)
        # Employers can see their employees' leave requests
        elif user.role == User.Role.EMPLOYER:
            queryset = queryset.filter(employee__employer_profile__user=user)
...
class LeaveRequestDetailView(generics.RetrieveUpdateDestroyAPIView):
    ...
    def perform_update(self, serializer):
        ...
        if user.role in [User.Role.EMPLOYER, User.Role.ADMIN] and 'status' in self.request.data:
